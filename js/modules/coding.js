let CodingModule = //do not change
    {
        name: "Code",
        ...
        init: function()
        {
            ...
            let coding_area = this.coding_area = new LiteGUI.Area({ id: "codearea", height: "100%"});
            this.root.appendChild( coding_area.root );
            coding_area.split("horizontal",[null,"50%"],true);

            let left_area = coding_area.getSection(0);
            ...
            //CODING
            let coding_tabs_widget = this.coding_tabs_widget = new CodingTabsWidget();
        ...
        onNewScript: function( node, type )
        {
            ...
            if(type == "Script")
            {
                let component = new LS.Components.Script();
            ...
            {
                let component = new LS.Components.ScriptFromFile();
                node.addComponent( component );
            }
            else if (type == "Global")
            {
        ...
        extractOptionsFromInstance: function( instance, options )
        {
            ...
            //compute id
            let fullpath = instance.fullpath || instance.filename; //for resources
            let uid = instance.uid || instance.name; //for components
            let id = options.id || fullpath || uid;
            options.id = id;
            ...
            if( !lang )
            {
                ...
                if( fullpath )
                {
                    let ext = LS.RM.getExtension(fullpath);
                    if( ext == "js" )
                        lang = "javascript";
                    else if( ext == "txt" )
                        lang = "text";
        ...
        findInstance: function( options, callback )
        {
            ...
            {
                let comp = LS.GlobalScene.findComponentByUId( id );
                if(callback)
        ...
        showCodingHelp: function( options )
        {
            ...
            let dialog = new LiteGUI.Dialog("info_message",help_options);
        ...
        onCodeError: function( e,err )
        {
            ...
            let tab = this.coding_tabs_widget.editInstanceCode( err.script );
            if(!tab || !tab.pad)
                return;
            ...
            let msg = String(err.msg || err.message || err.error);

            let elem = InterfaceModule.setStatusBar("<span class='link'>Error in code: " + msg + "</span>", "error" );
        ...
        onKeyDown: function(e)
        {
            ...
            let coding = this.coding_tabs_widget.root.querySelector(".CodeMirror");
            if(coding)
...
{
    let context_locator = component.getLocator() + "/context";
    let context = component.getContext();

    let icon = this.current_section.querySelector(".script-context-icon");
...
LS.Components.ScriptFromFile["@inspector"] = function( component, inspector )
{
    ...
    inspector.addButton(null,"Edit Code", { width: 100, callback: function() {
            ...
            if(!component.filename)
            {
                ...
                LiteGUI.prompt("Choose a filename", function(filename){
                    if(!filename)
                        return;
                    CodingModule.openTab();
                    let res = new LS.Resource();
                    let extension = LS.RM.getExtension(filename);
                    if(extension != "js")
                ...
                */
                DriveModule.showCreateScriptDialog({filename: "script.js"}, function(resource){
                    if(!resource)
                        return;
                    CodingModule.openTab();
                    let fullpath = resource.fullpath || resource.filename;
            ...
            CodingModule.openTab();
            let res = LS.ResourcesManager.load( component.filename, null, function(res){
                CodingModule.editInstanceCode( res );
            });
        }});
...
LS.Components.Script.prototype.onComponentInfo = function( widgets )
{
    ...
    let locator_widget = widgets.addString("Context Locator", this.getLocator() + "/context", { disabled: true } );
...
LiteGUI.Inspector.prototype.addCode = function( name, value, options )
{
    ...
    let uid = instance.uid || ("code_" + this.tab_index);
    ...
    else //embeded editor
    {
        ...
        let codepad = new CodingPadWidget();
...
LS.Components.Script.actions["convert_to_script"] = {
    ...
    callback: function() {
        ...
        LiteGUI.prompt("Choose a filename for the source file", function(v){
            ...
            let resource = new LS.Resource();
            resource.setData( code );
            LS.RM.registerResource( v, resource );
            info.filename = resource.filename;

            let index = node.getIndexOfComponent(compo);
            node.removeComponent(compo);

            let script = new LS.Components.ScriptFromFile();
...
LS.Components.ScriptFromFile.actions["convert_to_script"] = {
    ...
    callback: function() {
        ...
        let script = new LS.Components.Script();
        let index = node.getIndexOfComponent(this);
